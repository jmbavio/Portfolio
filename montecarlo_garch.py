...
Requiere: requests, arch, pandas, numpy, scipy, matplotlib
    pip install requests arch pandas numpy scipy matplotlib

Nota sobre la Etapa 1: la descarga usa `requests` directo contra la API de
Yahoo Finance en lugar de `yfinance`. `yfinance` depende de `curl_cffi` para
imitar un navegador y evitar el bloqueo anti-bot de Yahoo, pero ese cliente
falla (SSL "connection reset") detras de proxies que reterminan TLS (como el
de los sandboxes de Claude Code). `requests` con TLS estandar sí funciona en
ese escenario.
"""
...
DIAS_POR_PERIODO = {"y": 365, "mo": 30, "d": 1}


def _periodo_a_segundos(period):
    unidad = "mo" if period.endswith("mo") else period[-1]
    cantidad = int(period[: -len(unidad)])
    return cantidad * DIAS_POR_PERIODO[unidad] * 86_400


def _descargar_ticker(ticker, period="3y", max_reintentos=5):
    """Baja el cierre ajustado diario de un ticker via la API de Yahoo Finance."""
    ahora = int(time.time())
    params = {
        "period1": ahora - _periodo_a_segundos(period),
        "period2": ahora,
        "interval": "1d",
        "events": "div,splits",
    }
    ...
    for intento in range(max_reintentos):
        ...
        if resp.status_code == 429 and intento < max_reintentos - 1:
            print(f"[AVISO] {ticker}: rate-limit de Yahoo (429), reintentando en {espera}s...")
            time.sleep(espera)
            espera *= 2
            continue
        resp.raise_for_status()
    else:
        raise RuntimeError(f"No se pudo descargar {ticker} tras {max_reintentos} intentos (rate-limit).")

    resultado = resp.json()["chart"]["result"]
    if not resultado:
        raise RuntimeError(f"Yahoo Finance no devolvio datos para {ticker}.")

    datos = resultado[0]
    timestamps = datos["timestamp"]
    cierre_ajustado = datos["indicators"]["adjclose"][0]["adjclose"]

    fechas = pd.to_datetime(timestamps, unit="s").normalize()
    serie = pd.Series(cierre_ajustado, index=fechas, name=ticker)
    return serie[~serie.index.duplicated(keep="last")].dropna()


def descargar_retornos(tickers, period="3y", pausa_entre_tickers=0.5):
    """Baja precios de cierre ajustado y devuelve retornos log diarios en %."""
    series = []
    for ticker in tickers:
        series.append(_descargar_ticker(ticker, period=period))
        time.sleep(pausa_entre_tickers)  # evitar rate-limit de Yahoo
...
def main():
    ...
    print("=== Etapa 1: descarga de datos (Yahoo Finance API via requests, 3 años) ===")
